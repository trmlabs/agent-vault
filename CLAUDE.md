...
- **Single ingress into the broker — transparent MITM** (on by default, port 14322, disable with `--mitm-port 0`): plain HTTP forward-proxy ingress backed by [internal/mitm](internal/mitm/) + [internal/ca](internal/ca/) (software CA, root key encrypted with the master key). The listener accepts both `CONNECT host:port` (HTTPS upstreams) and absolute-form forward-proxy requests (`POST http://host/path HTTP/1.1`, RFC 7230 §5.3.2) for plain-HTTP upstreams on the same port. Clients use `HTTPS_PROXY=http://...` and `HTTP_PROXY=http://...` — both point at the same proxy URL. Deploy on a trusted/private network. Credential injection lives in `brokercore`. HTTP/1.1 at the ingress, with transparent WebSocket upgrade support (HTTP/2 not yet). Bind failures are non-fatal in the legacy profile; the strict credential-proxy profile treats proxy initialization or bind failure as fatal.
- **Strict credential-proxy profile**: `AGENT_VAULT_CREDENTIAL_PROXY=true` requires request-time Vault reads, durable audit admission, loopback listeners behind verified encrypted ingress and `AGENT_VAULT_WORKLOAD_IDENTITY_FILE`. HTTP and PostgreSQL share live Kubernetes pod-proof verification without a standing-token fallback. The initial HTTP surface is HTTPS GET/HEAD without caller query/body and with configured header placeholders only. An exact GET service may supply an operator-configured literal `fixed_query`; legacy forwarding and HEAD reject that service. Header support includes Basic authentication with a placeholder username and empty password. See [the release profile](examples/credential-proxy/README.md) and [identity setup](examples/credential-proxy/kubernetes/README.md); deployed isolation remains a separate acceptance requirement.
- **Trusted per-task relay**: `agent-vault task-relay --config FILE` runs in a separate trusted Kubernetes Pod. It matches the actual socket IP to an operator-pinned live sandbox Pod UID and fixed policy, then authenticates to the broker with relay-only projected proofs. The sandbox has public settings/placeholders only. HTTP CONNECT, PostgreSQL and optional fixed browser actions use native TLS; pair loss, API failure, audit failure and deadline stop the task. See [the task relay guide](examples/credential-proxy/task-relay/README.md) for configuration, protocol limits and required deployed CNI acceptance. This does not establish identity for arbitrary hosted-agent runtimes.
