...
Run one relay per managed Kubernetes task in a separate trusted Pod. The sandbox
uses public connection settings and placeholders; identity proofs remain in the
relay. The relay checks the real socket peer against an operator-pinned live Pod
UID before using its fixed HTTP, database or optional browser policy.

| Flag | Default | Description |
|------|---------|-------------|
| `--config` | Required | Operator-owned JSON containing task ID, deadline, sandbox namespace/name/UID/IP, Kubernetes API trust and reviewer token path, listener certificate/key, durable audit path and fixed protocol destinations/proof paths. |

No environment flags configure this command. The deadline must be in the future
and at most eight hours away. Native TLS listeners preserve socket identity;
proofs, private keys and audit storage must not be mounted in the sandbox.
Pair loss, API failure, audit failure or deadline stops active work. A browser
cleanup failure returns an error; broker expiry remains the crash fallback.

The checkout's `examples/credential-proxy/task-relay/` contains the complete
operator guide, `config.example.json`, client contracts and acceptance record.
Its example is deliberately expired. Local tests do not establish deployed
network isolation or support for other hosted-agent runtimes.
...
<AccordionGroup>
  <Accordion title="agent-vault server">
    ...
    **Environment variables:**
    ...
    | `AGENT_VAULT_CREDENTIAL_PROXY` | Optional boolean, default `false`. Enables the restricted release profile with request-time Vault reads, fail-closed forwarding and durable audit admission. Requires workload identity, verified Vault TLS (loopback HTTP is allowed for local fixtures), and loopback listeners behind protected ingress. See [Credential proxy profile](/self-hosting/environment-variables#credential-proxy-profile). |
    | `AGENT_VAULT_WORKLOAD_IDENTITY_FILE` | Path to a JSON Kubernetes workload policy. Enables the same exclusive projected-token verifier on HTTP and PostgreSQL proxy admission, without a standing-token fallback. Required by `AGENT_VAULT_CREDENTIAL_PROXY=true`; management authentication stays separate. |
